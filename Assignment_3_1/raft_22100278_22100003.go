...
func (rf *Raft) handleNewHeartbeat() {
	...
	// Start thread for being able to recieve commands
	go rf.handleClientCommand()

	// Start thread for each follower to be able to send new logs
	for i := 0; i < len(rf.peers); i++ {
		if i != rf.me {
			go rf.handleBulkSendAppedEntries(i)
		}
	}
	rf.mu.Unlock()
...
// A Thread that runs for a particular "targetServer", which calls handleSentAppendEntries
// Whenever nextIndex and Leaders log changes
func (rf *Raft) handleBulkSendAppedEntries(targetServer int) {
	for true {
		rf.mu.Lock()
		if rf.role != Leader {
			// If node is no longer the Leader -> terminate loop, which terminates the thread
		...
		if rf.nextIndex[targetServer] > rf.matchIndex[targetServer] && len(rf.logs) > rf.nextIndex[targetServer] {
			// If nextIndex != matchIndex AND the log at nextIndex actually exists -> Call handleSendAppendEntries
			numLogs := len(rf.logs) - rf.nextIndex[targetServer]
			thisLogArr := make([]Log, 0, numLogs)
			for i := rf.nextIndex[targetServer]; i < rf.nextIndex[targetServer]+numLogs; i++ {
				thisLogArr = append(thisLogArr, rf.logs[i])
			}
			argsAE := AppendEntriesArgs{rf.currentTerm, rf.me, rf.commitIndex, -1, -1, thisLogArr}
			if rf.nextIndex[targetServer] > 0 {
				argsAE.LeaderPrevLogIndex = rf.nextIndex[targetServer] - 1
				argsAE.LeaderPrevLogTerm = rf.logs[argsAE.LeaderPrevLogIndex].Term
			}
			rf.mu.Unlock()
			rf.handleSendAppendEntries(targetServer, argsAE)
			rf.mu.Lock()
		}
		rf.mu.Unlock()
	}
...
// Makes sure if more than 1 command is buffered into the append channel in one instant
// Applies 150 milliseconds gap
func (rf *Raft) handleClientCommand() {
	for true {
		...
		select {
		case thisLog := <-rf.appendCh:
			for true {
				rf.mu.Lock()
				if time.Now().Sub(rf.timestamp).Seconds() > 0.15 {
					rf.mu.Unlock()
					break
				}
				rf.mu.Unlock()
			...
			rf.logs = append(rf.logs, thisLog)
			rf.mu.Unlock()
		}
	}
}
